...
class RolePanel extends React.Component<Props, State> {
  ...
  async loginOut() {
    ...
    // const resp = await request(`http://139.129.229.99:8086/jeecg-boot//intf/location/logout`, {
    const resp = await request(`${BASE_API_URL}/jeecg-boot//intf/location/logout`, {
      method: 'GET',
      ...
      <div className={styles.role_panel_box}>
        <Dropdown overlay={menu} trigger={['click']}>
          <span className={styles.role_panel_box_span}>
            {this.props.showName || this.state.userInfo.name}
...
const mapState = ({ commonState }, { showName }) => {
